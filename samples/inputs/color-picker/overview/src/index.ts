import {
    defineComponents,
    IgcButtonComponent,
    IgcCardComponent,
    IgcColorPickerComponent,
    IgcIconButtonComponent,
    IgcSelectComponent,
    IgcSelectItemComponent,
    registerIconFromText,
} from 'igniteui-webcomponents';
import 'igniteui-webcomponents/themes/light/material.css';
import "./index.css";

defineComponents(
    IgcButtonComponent,
    IgcCardComponent,
    IgcColorPickerComponent,
    IgcIconButtonComponent,
    IgcSelectComponent,
    IgcSelectItemComponent
);

const refreshIcon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M17.65 6.35A7.958 7.958 0 0 0 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08a5.99 5.99 0 0 1-5.65 4c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>';

export class ColorPickerOverview {
    constructor() {
        registerIconFromText('refresh', refreshIcon, 'material');

        const picker = document.querySelector('igc-color-picker') as IgcColorPickerComponent;
        const reset = document.querySelector('#colorReset') as HTMLElement;

        reset.addEventListener('click', () => {
            picker.value = '';
        });
    }
}

new ColorPickerOverview();
